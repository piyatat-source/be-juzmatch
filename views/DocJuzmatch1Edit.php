...
loadjs.ready(["wrapper", "head"], function () {
    ...
    fdoc_juzmatch1edit.lists.status = <?= $Page->status->toClientList($Page) ?>;
...
<span id="el_doc_juzmatch1_status">
    <select
        id="x_status"
        name="x_status"
        class="form-select ew-select<?= $Page->status->isInvalidClass() ?>"
        data-select2-id="fdoc_juzmatch1edit_x_status"
        data-table="doc_juzmatch1"
        data-field="x_status"
...
<script>
loadjs.ready("fdoc_juzmatch1edit", function() {
    var options = { name: "x_status", selectId: "fdoc_juzmatch1edit_x_status" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdoc_juzmatch1edit.lists.status.lookupOptions.length) {
        options.data = { id: "x_status", form: "fdoc_juzmatch1edit" };
    } else {
        options.ajax = { id: "x_status", form: "fdoc_juzmatch1edit", limit: ew.LOOKUP_PAGE_SIZE };
    }
